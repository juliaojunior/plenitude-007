import Link from "next/link"
import { notFound } from "next/navigation"
import { ChevronLeft } from "lucide-react"
import { CATEGORIAS, getCategoria } from "@/lib/categorias"
import { db } from "@/lib/db"
import { meditacoes } from "@/lib/db/schema"
import { eq } from "drizzle-orm"

export const revalidate = 3600

export async function generateStaticParams() {
  return CATEGORIAS.map((c) => ({ categoria: c.slug }))
}

interface Props {
  params: Promise<{ categoria: string }>
}

export default async function CategoriaPage({ params }: Props) {
  const { categoria } = await params
  const cat = getCategoria(categoria)
  if (!cat || !CATEGORIAS.find((c) => c.slug === categoria)) notFound()

  const lista = await db
    .select()
    .from(meditacoes)
    .where(eq(meditacoes.categoria, categoria))
    .orderBy(meditacoes.titulo)

  return (
    <div className="mx-auto max-w-lg px-4 pt-4">
      {/* Back + header */}
      <Link
        href="/meditacoes"
        className="mb-4 inline-flex items-center gap-1 text-sm text-[var(--text-muted)] hover:text-[var(--text)]"
      >
        <ChevronLeft size={16} /> Meditações
      </Link>

      <div className="mb-6 flex items-center gap-3">
        <span
          className="flex h-12 w-12 items-center justify-center rounded-xl text-2xl"
          style={{ backgroundColor: cat.cor + "30" }}
        >
          {cat.emoji}
        </span>
        <div>
          <h1 className="text-2xl font-semibold text-[var(--text)]">{cat.label}</h1>
          <p className="text-sm text-[var(--text-muted)]">{lista.length} meditações</p>
        </div>
      </div>

      {lista.length === 0 ? (
        <div className="py-16 text-center">
          <p className="text-[var(--text-muted)]">Nenhuma meditação ainda.</p>
          <p className="mt-1 text-xs text-[var(--text-faint)]">Em breve novos conteúdos.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {lista.map((med) => (
            <Link
              key={med.id}
              href={`/meditacoes/${categoria}/${med.id}`}
              className="group rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4 transition-all hover:border-[var(--gold)]/40 hover:shadow-md active:scale-[0.99]"
            >
              <p className="font-semibold text-[var(--text)]">{med.titulo}</p>
              {med.textoBiblico && (
                <p className="mt-1 line-clamp-2 font-display text-sm italic text-[var(--text-muted)]">
                  &ldquo;{med.textoBiblico}&rdquo;
                </p>
              )}
              {med.duracaoSegundos && (
                <p className="mt-2 text-xs text-[var(--text-faint)]">
                  {Math.ceil(med.duracaoSegundos / 60)} min
                </p>
              )}
            </Link>
          ))}
        </div>
      )}
    </div>
  )
}
